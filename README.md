...
**환경별 시뮬레이션**
- 야경, 숲속, 단풍길 등 다양한 배경
- "이 환경에서 이 차는 어떤 느낌일까?" 상상해볼 수 있도록
...
**과도한 Props Drilling (포트폴리오에서 자세히 확인 가능)**
- **문제**: 최상위 컴포넌트에서 최하위 컴포넌트까지 Props가 전달되며 결합도 증가 및 유지보수 어려움 발생
- **해결**: Context를 활용해 props 최소화

**OrbitControls 기반 잦은 리렌더로 인한 성능 이슈 (포트폴리오에서 자세히 확인 가능)**
- **문제**: 자유롭게 확대,축소 및 화면 전환으로 인해 부모 컴포넌트 자체의 잦은 리렌더 발생
- **해결**: React.memo, useCallback으로 최적화


## Tailwind 관련

### 8. Tailwind 동적 클래스명 적용 문제
